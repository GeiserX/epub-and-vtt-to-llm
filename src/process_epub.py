...
def process_epub(file_path, catalog_db_conn, LANG):
    texts = []
    ...
    try:
        # Step 1: Extract contents of the epub file
        ...
        try:
            ...
            try:
                ...
                cursor.execute("""
                    SELECT LanguageIndex, Symbol, Year, Number
                    FROM Publication
                    LIMIT 1;
                """)
                row = cursor.fetchone()
                ...
                language_index, symbol, year, issue_tag_number = row

                logging.debug(f"LanguageIndex: {language_index}")
                logging.debug(f"Symbol: '{symbol}'")
                logging.debug(f"Year: {year}")
                logging.debug(f"Number: {issue_tag_number}")

                # Build the hash input string
                fields = []
                try:
                    fields.append(str(language_index).strip())
                except Exception as e:
                    logging.error(f"Error getting LanguageIndex: {e}")
                    return texts
                try:
                ...
                content_entries = []
                for table_name in ['Document', 'Chapter', 'Verse']:
                    try:
                        cursor.execute(f"""
                ...
                # Now, get publication data from catalog.db using LanguageIndex, Symbol, Number
                catalog_cursor = catalog_db_conn.cursor()

                # Adjust Number for querying; if None or 0, set to 0
                ...
                catalog_cursor.execute("""
                    SELECT 
                        ...
                        Id
                    FROM Publication
                    WHERE LanguageId=? AND Symbol=? AND (Number=? OR Number IS NULL)
                    LIMIT 1;
                """, (language_index, symbol, issue_tag_number_for_query))

                pub_row = catalog_cursor.fetchone()
                if not pub_row:
                    logging.warning(f"No matching publication found in catalog.db for LanguageId={language_index}, Symbol='{symbol}', Number='{issue_tag_number_for_query}'")
                    conn.close()
                    return texts
                ...
                for idx, (document_id, encrypted_content) in enumerate(content_entries):
                    ...
                    try:
                        ...
                        data_entry = {
                            ...
                            'language': LANG,  # Add language from LANG variable
                            'text': paragraph_text
                        }
...
if __name__ == '__main__':
    ...
    # Get LANG from environment variable or default to 'S' for Spanish
    LANG = os.environ.get('LANG', 'S')
    # Set paths
    default_epubs_directory = "E:/epubs/"  # Update this path to your .epub files directory
    default_output_file_path = "E:/epubs/epubs.json"
    default_catalog_db_path = os.path.join(default_epubs_directory, "catalog.db")

    # Set up argument parser
    parser = argparse.ArgumentParser(description='Process .epub files and extract texts.')
    parser.add_argument('--directory', type=str, default=default_epubs_directory,
                        help=f'Directory containing .epub files (default: {default_epubs_directory})')
    ...
    if not os.path.exists(directory):
        ...
        # Iterate over all .epub files in the directory
        for filename in os.listdir(directory):
            if filename.endswith('.epub'):
                epub_file = os.path.join(directory, filename)
                # Process the .epub file and extract texts
                texts = process_epub(epub_file, catalog_db_conn, LANG)
                if texts:
                    all_texts.extend(texts)
                else:
                    logging.warning(f"No texts were extracted from {epub_file}.")
        ...
        if all_texts:
            ...
            logging.warning("No texts were extracted from any .epub files.")