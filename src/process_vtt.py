    ...
    with open(sjson_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if data.get('type') == 'media-item':
                o = data.get('o', {})
                # Collect all keys except 'images', 'checksums', 'key', and 'date'
                keys = set(o.keys()) - {'images', 'checksums', 'key', 'date'}
                # Include keys from 'key'
                key = o.get('key', {})
        ...
        for line in f:
            ...
            if data.get('type') == 'media-item':
                ...
                if naturalKey:
                    ...
                    # Extract metadata fields excluding 'images', 'checksums', 'key', and 'date'
                    for key in o.keys():
                        if key not in {'images', 'checksums', 'key', 'date'}:
                            metadata[key] = o.get(key)

                    # Include fields from 'key' at the top level
                    key = o.get('key', {})
...
# Function to build filename to naturalKey mapping from media.db
def build_filename_mapping(db_path):
    ...
    try:
        ...
        elif 'pub' in column_names:
            id_column = 'pub'
        else:
            logging.error("Neither 'identifier' nor 'pub' column found in downloaded_vtts table.")
        ...
        for identifier, track, formatCode, vtt_url in rows:
            ...
            # Reconstruct the naturalKey without relying on LANG
            ...
            if isinstance(identifier, str) and not identifier.isdigit():
                # Assuming 'pub' case
                if languageCode:
                    naturalKey = f"a-{identifier}_{languageCode}_{track}_{formatCode_upper}"
                else:
                    naturalKey = f"a-{identifier}_{track}_{formatCode_upper}"
            else:
                # Assuming 'docID' case
                if languageCode:
                    naturalKey = f"b-{identifier}_{languageCode}_{track}_{formatCode_upper}"
                else:
                    naturalKey = f"b-{identifier}_{track}_{formatCode_upper}"
...
if __name__ == '__main__':
    ...
    directory = "E:/vtts"  # Update this path to your .vtt files directory
    db_path = "E:/vtts/media.db"  # Path to the media.db database
    ...
    print("Building filename to naturalKey mapping from media.db...")
